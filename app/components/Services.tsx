...
export default function Services() {
  const categories = [
    ...
    },
    {
      ...
      className: "z-[20] -mt-8 ml-8 md:ml-16" 
    },
    {
      label: "Motion Design",
    ...
    <section id="services" className="bg-neutral-950 py-24 px-6 overflow-hidden">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start justify-center">
          ...
          {/* Lado Esquerdo: Foto da Blusa (Limpo, sem onError) */}
          ...
          {/* Lado Direito: Cards Largos e Escadinha Suave */}
          <div className="w-full lg:w-1/2 flex flex-col relative pt-4">
            {categories.map((cat, idx) => (
              <div 
                key={idx}
                className={`bg-[#FDFCF7] p-8 md:p-10 rounded-xl shadow-[0_35px_60px_-15px_rgba(0,0,0,0.6)] w-full max-w-[540px] border border-white/5 ${cat.className}`}
              >
                {/* Badge Azul */}
                <span className="inline-block bg-[#436AFA] text-white text-[10px] font-bold px-5 py-2 rounded-full mb-8 uppercase tracking-widest shadow-md">
                  {cat.label}
                </span>

                <ul className="space-y-4">
                  {cat.items.map((item, i) => (
                    <li key={i} className="flex items-start text-neutral-600 font-medium text-sm md:text-base leading-snug tracking-tight">
